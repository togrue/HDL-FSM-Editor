    ...
    def __init__(self, language, number_of_files, readfile, generate_path, module_name):
        ...
        if self.__hdl_is_up_to_date(readfile, hdlfilename, hdlfilename_architecture, show_message=False):
            ...
            try:
                ...
                messagebox.showerror("Error in HDL-FSM-Editor", "File " + hdlfilename + " could not be opened for copying into HDL-Tab.")
            if number_of_files==2:
                ...
                try:
                    ...
                    messagebox.showerror("Error in HDL-FSM-Editor", "File " + hdlfilename + " (architecture-file) could not be opened for copying into HDL-Tab.")
    ...
    def __hdl_is_up_to_date(self, path_name, hdlfilename, hdlfilename_architecture, show_message):
        ...
        if hdlfilename_architecture is not None:
            if not os.path.isfile(hdlfilename_architecture):
                if show_message:
                    messagebox.showerror("Error in HDL-FSM-Editor", "The architecture file\n" + hdlfilename_architecture + " is missing.")
                return False
