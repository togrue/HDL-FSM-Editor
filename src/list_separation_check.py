...
class ListSeparationCheck:
    def __init__(self, list_string, language):
        ...
        if language == "VHDL":
            ...
            separator = ","
            comment_identifier = "//"
        list_string_without_block_comment = (
            hdl_generation_library.remove_vhdl_block_comments(list_string)
        )
        list_string_without_comments = self.__replace_all_comments_at_line_end(
    ...
    def __replace_all_comments_at_line_end(
        self, list_string_without_block_comment, comment_identifier
    ):
        list_array = list_string_without_block_comment.split("\n")
        list_string_without_comments = ""
        for line in list_array:
            list_string_without_comments += (
                self.__replace_comment_at_line_end_by_blank(comment_identifier, line)
                + "\n"
            )
    ...
    def __remove_character_by_blank(self, index):
        if index == 0:
            ...
            self.list_string = (
                self.list_string[: -index - 1] + " " + self.list_string[-index:]
            )
