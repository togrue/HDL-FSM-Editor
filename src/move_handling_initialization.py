...
def move_initialization_overlapping(event, event_x, event_y):
    items_near_mouse_click_location = (
        create_a_list_of_overlapping_items_near_the_mouse_click_location(
            event_x, event_y
        )
    )
    ...
    if move_list:  # If no bug exists, then movelist is never empty. So this check is only an "insurance".
        ...
        move_do_funcid = main_window.canvas.bind(
            "<Motion>",
            lambda event, move_list=move_list: move_handling.move_do(
        ...
        main_window.canvas.bind(
            ...
            lambda event,
            move_list=move_list,
            move_do_funcid=move_do_funcid: move_handling_finish.move_finish(
...
def create_move_list(items_near_mouse_click_location, event_x, event_y):
    move_list = []
    move_list_entry_for_diagram_object = (
        create_move_list_entry_if_a_diagram_object_is_moved(
            items_near_mouse_click_location
        )
    )
...
def create_move_list_entry_if_a_diagram_object_is_moved(
    items_near_mouse_click_location,
):
...
def add_lines_connected_to_the_diagram_object_to_the_list(move_list):
    ...
    for tag in tag_list_of_object_to_move:  # Check which Canvas lines are "connected" and must be moved together with the diagram object.
        ...
        if tag.endswith("_start"):
            tag_of_connected_line = tag[
                :-6
            ]  # transition<n>, state<n>_comment_line, connection<n>
            to_be_moved_point_of_connected_line = "start"
        elif tag.endswith("_end"):
            tag_of_connected_line = tag[
                :-4
            ]  # transition<n>, state<n>_comment_line, connection<n>, ca_connection<n>
        ...
        if to_be_moved_point_of_connected_line != "":
            ...
            id_of_connected_line = main_window.canvas.find_withtag(
                tag_of_connected_line
            )[0]
            move_list.append(
                [id_of_connected_line, to_be_moved_point_of_connected_line]
            )
            transition_handling.extend_transition_to_state_middle_points(
                tag_of_connected_line
            )
...
def get_move_list_entry_for_line_of_condition_action_block(transition_tag):
    ...
    for t in transition_tags:
        if t.startswith(
            "ca_connection"
        ):  # To this transition, a condition_action block is connected.
            ...
            return [
                ...
                "end",
                transition_tag,
            ]  # The transition_tag is needed in move_finish.
    return []


def add_items_for_moving_a_single_line_point_to_the_list(
    move_list, items_near_mouse_click_location, event_x, event_y
):
    ...
    if transition_tags != ():
        ...
        for tag in transition_tags:
            if tag.startswith("transition"):
                ...
                remove_tags_and_hide_priority(
                    line_id, tag, transition_tags, moving_point
                )
...
def find_the_item_id_of_the_line(items_near_mouse_click_location):
    for item_id in items_near_mouse_click_location:
        if main_window.canvas.type(
            item_id
        ) == "line" and "grid_line" not in main_window.canvas.gettags(item_id):
            return item_id
    return None
...
def remove_tags_and_hide_priority(
    line_id, transition_tag, transition_tags, moving_point
):
    for tag in transition_tags:
        if moving_point == "start" and tag.startswith("coming_from_"):
            main_window.canvas.dtag(
                line_id, tag
            )  # delete the "coming_from_" tag from the line
            ...
            if len(priority_dict) == 1:
                for outgoing_transition in priority_dict:
                    main_window.canvas.itemconfigure(
                        outgoing_transition + "priority", state=tk.HIDDEN
                    )
                    main_window.canvas.itemconfigure(
                        outgoing_transition + "rectangle", state=tk.HIDDEN
                    )
            ...
            main_window.canvas.dtag(
                line_id, tag
            )  # delete the "going_to_" tag from the line
