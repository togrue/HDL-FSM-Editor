...
def _get_complete_design_as_text_object():
    ...
    design += "connector_number|" + str(connector_insertion.ConnectorInstance.connector_number) + "\n"
...
def _set_diagram_to_version_selected_by_stack_pointer() -> None:
    ...
    while _line_index < len(lines):
        if lines[_line_index].startswith("state_number|"):
            ...
            connector_insertion.ConnectorInstance.connector_number = int(rest_of_line)
            ...
            if not is_priority_rectangle:
                rectangle_id = connector_insertion.ConnectorInstance.draw_connector(coords, tags)
