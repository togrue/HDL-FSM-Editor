...
class GlobalActions:
    ...
    def __init__(self, menu_x, menu_y, height, width, padding):
        ...
        self.text_before_id.bind(
            "<Control-z>", lambda event: self.text_before_id.undo()
        )
        self.text_before_id.bind(
            "<Control-Z>", lambda event: self.text_before_id.redo()
        )
    ...
    def tag(self):
        main_window.canvas.itemconfigure(
            self.window_id,
            tag="global_actions" + str(GlobalActions.global_actions_number),
        )
