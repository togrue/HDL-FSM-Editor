...
def insert_connector(event):
    ...
    main_window.canvas.tag_bind(
        connector_id,
        "<Enter>",
        lambda event, id=connector_id: main_window.canvas.itemconfig(id, width=2),
    )
    main_window.canvas.tag_bind(
        connector_id,
        "<Leave>",
        lambda event, id=connector_id: main_window.canvas.itemconfig(id, width=1),
    )
...
def move_to(event_x, event_y, rectangle_id, first, last):
    ...
    new_upper_left_corner = calculate_new_upper_left_corner_of_the_rectangle(
        event_x, event_y, edge_length
    )
    new_lower_right_corner = calculate_new_lower_right_corner_of_the_rectangle(
        event_x, event_y, edge_length
    )
    move_rectangle_in_canvas(
        rectangle_id, new_upper_left_corner, new_lower_right_corner
    )
