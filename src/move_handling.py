    ...
    for entry in move_list:
        ...
        if item_type == "oval":
            ...
            transition_handling.move_to(
                event_x, event_y, item_id, item_point_to_move, first, move_list, last
            )
            ...
            if item_id in state_action_handling.MyText.mytext_dict:
                ...
                item_id
                in global_actions_combinatorial.GlobalActionsCombinatorial.dictionary
            ):
                ref = (
...
def get_exact_or_rounded_canvas_coordinates(event):
    if event.type == "5":  # ButtonRelease
        return canvas_editing.translate_window_event_coordinates_in_rounded_canvas_coordinates(
            event
        )
    else:  # Motion
        return canvas_editing.translate_window_event_coordinates_in_exact_canvas_coordinates(
            event
        )
    ...
    for entry in move_list:
        ...
        if main_window.canvas.type(moved_item_id) == "oval":
            ...
            event_x_mod, event_y_mod = (
                event_x + state_handling.difference_x,
...
def connector_moved_too_close_to_other_object(move_list, event_x, event_y):
    for entry in move_list:
        ...
        if (
            ...
            and main_window.canvas.itemcget(moved_item_id, "fill")
            == constants.CONNECTOR_COLOR
        ):
            # Keep the distance between event and anchor point constant:
            event_x_mod, event_y_mod = (
            ...
            new_upper_left_corner = [
                event_x_mod - edge_length / 2,
                event_y_mod - edge_length / 2,
            ]
            ...
            for overlapping_item in overlapping_list:
                if overlapping_item != moved_item_id and (
                    ...
                    or (
                        ...
                        and main_window.canvas.itemcget(overlapping_item, "fill")
                        == constants.CONNECTOR_COLOR
                    )
                ):
                    return True
