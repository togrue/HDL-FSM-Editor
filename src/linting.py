...
def recreate_keyword_list_of_unused_signals():
    ...
    ) in custom_text.CustomText.read_variables_of_all_windows.items():
    ...
    read_variables = [
        value for value in read_variables if value not in ("true", "false")
    ]
    ...
    for output in main_window.interface_ports_text.writable_ports_list:
        if output in written_variables:
            written_variables = [
                value for value in written_variables if value != output
            ]
        else:
            main_window.keywords["not_written"].append(output)
        if (
            main_window.language.get() != "VHDL" and output in read_variables
        ):  # A Verilog output can be read.
    ...
    for signal in (
        ...
        if signal in written_variables and signal in read_variables:
            written_variables = [
                value for value in written_variables if value != signal
            ]  # remove signal from list
            read_variables = [
                value for value in read_variables if value != signal
            ]  # remove signal from list
    ...
    for constant in (
        ...
        written_variables = [
            value for value in written_variables if value != constant
        ]  # remove constant from list
        if constant in read_variables:
            read_variables = [
                value for value in read_variables if value != constant
            ]  # remove constant from list
        else:
    ...
    for port_type in main_window.interface_ports_text.port_types_list:
        if port_type in read_variables:
            read_variables = [
                value for value in read_variables if value != port_type
            ]  # remove signal from list
    for generic in main_window.interface_generics_text.generics_list:
        written_variables = [
            value for value in written_variables if value != generic
        ]  # remove generic from list
        if generic in read_variables:
            read_variables = [
                value for value in read_variables if value != generic
            ]  # remove signal from list
...
def update_highlight_tags_in_all_windows_for_not_read_not_written_and_comment():
    ...
    for text_id in custom_text.CustomText.read_variables_of_all_windows:
        text_id.update_highlight_tags(
            canvas_editing.fontsize, ["not_read", "not_written", "comment"]
        )
    text_ids_fixed = [
