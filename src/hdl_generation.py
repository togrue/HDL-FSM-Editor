...
def design_is_not_ready_for_hdl_generation():
    ...
    if name.isspace() or name=="":
        messagebox.showerror("Error in HDL-FSM-Editor", 'No module-name is specified,\nso no HDL-generation is possible.')
        return True
    path_string = main_window.generate_path_value.get()
    if path_string.isspace() or path_string=="":
        messagebox.showerror("Error in HDL-FSM-Editor", 'No path for the generation of the HDL-files is specified,\nso no HDL-generation is possible.')
        return True
    else:
        path = Path(path_string)
        if not path.exists():
            messagebox.showerror("Error in HDL-FSM-Editor", 'HDL-generation for module ' + name +
    ...
    if reset.isspace() or reset=="":
        messagebox.showerror("Error in HDL-FSM-Editor", 'No reset signal name is specified,\nso no HDL-generation is possible.')
        return True
    clock = main_window.clock_signal_name.get()
    if clock.isspace() or clock=="":
        messagebox.showerror("Error in HDL-FSM-Editor", 'No clock signal name is specified,\nso no HDL-generation is possible.')
        return True
    return False
...
def create_hdl(header, write_to_file):
    ...
    if main_window.language.get()=="VHDL":
        ...
        architecture = hdl_generation_architecture.create_architecture(file_to_use, file_line_number_to_use, state_tag_list_sorted)
    else:
                    ...
                    if first_line_of_state_comments=="":
                        ...
                        if not first_line_is_a_number:
                            ...
                            if int(first_line_of_state_comments) in state_tag_dict_with_prio:
                                state_tag_list.append(tag)
                                messagebox.showwarning("Warning in HDL-FSM-Editor",
                                                        "The state '" + main_window.canvas.itemcget(tag+"_name", "text") +
                                                        "' uses the order-number " + first_line_of_state_comments +
