...
def get_transition_target_condition_action(transition_tag):
    ...
    for tag in tags:
        if tag.startswith("going_to_state"):
            ...
            transition_target = main_window.canvas.itemcget(
                transition_target_tag + "_name", "text"
            )
            ...
            condition_action_canvas_item_id = main_window.canvas.find_withtag(
...
def extract_transition_specifications_from_the_graph(state_tag_list_sorted):
    ...
    for state_tag in state_tag_list_sorted:
        ...
        if state_tag + "_comment_line_end" in all_tags_of_state:
            canvas_id_of_comment_window = main_window.canvas.find_withtag(
                state_tag + "_comment"
            )[0]
            reference_to_state_comment_window = state_comment.StateComment.dictionary[
                canvas_id_of_comment_window
            ]
            ...
            state_comments = re.sub(
                r"^\s*[0-9]*\s*", "", state_comments
            )  # Remove order comment at comment start.
            if state_comments == "":
        ...
        for if_identifier in action_target_if_dictionary:
            if (
                ...
                if (
                    ...
                    for action_target_dict in action_target_array[state_name][
                        if_identifier
                    ]:
...
def create_action_and_branch_array_for_each_if_construct(transition_specifications):
    ...
    for transition_specification in transition_specifications:
        if transition_specification["command"] == "when":
            ...
            branchnumber_array_of_state[transition_specification["if_identifier"]] = (
                transition_specification["branch_number"]
            )
    if (
        action_target_array_of_state
...
def check_for_wrong_priorities(trace_array):
    ...
    for index, condition_sequence in enumerate(condition_array):
        if index < len(condition_array) - 1:
            if (
                condition_sequence
                == condition_array[index + 1][0 : len(condition_sequence)]
    ...
    for trace_index, trace in enumerate(traces_of_a_state_reversed):
        ...
        if (
            ...
            if trace:  # An empty trace may happen, when the transition with lowest priority has no condition and action (and has a connector?!).
                ...
                if (
                    trace_index
                    != 0  # This trace is not the trace with the lowest priority
                ...
                if trace[0]["command"] == "action":
                    ...
                    traces_of_a_state_reversed[trace_index + 1] = (
                        traces_of_a_state_reversed[trace_index + 1][:-1]
                    )  # remove endif
                    ...
                    target_at_error = ""
                    while (
                        trace[search_index]["command"]
                        + trace[search_index]["condition"]
                    ) == (
                        traces_of_a_state_reversed[trace_index + 1][search_index][
                            "command"
                        ]
                        + traces_of_a_state_reversed[trace_index + 1][search_index][
                            "condition"
    ...
    for state_tag in list_of_all_state_tags:
        ...
        if list_of_canvas_ids:
            ...
            if state_comments_list:
                ...
                first_line_is_a_number = bool(
                    all(c in "0123456789" for c in first_line_of_state_comments)
                )
                if first_line_is_a_number:
                    state_tag_dict_with_prio[int(first_line_of_state_comments)] = (
                        state_tag
                    )
                else:
...
def check_if_condition_is_a_comment(transition_condition):
    ...
    if (
        transition_condition_without_comments == ""
        or transition_condition_without_comments.isspace()
    ):
        return True
    return False
...
def create_outgoing_transition_list_with_priority_information(state_tag):
    ...
    for tag in all_tags_of_the_state:
        if tag.endswith("_start"):
            ...
            transition_priority_string = main_window.canvas.itemcget(
                transition_priority_text_tag, "text"
            )
            transition_tag_and_priority.append(
                [transition_tag, transition_priority_string]
            )
...
def create_concurrent_actions():
    if global_actions_handling.global_actions_combinatorial_number == 1:
        canvas_item_id = main_window.canvas.find_withtag(
...
def convert_hdl_lines_into_a_searchable_string(text):
    ...
    separated = re.sub(
        "<  =", "<=", separated
    )  # restore this operator (assignment or comparison)
    separated = re.sub(">  =", ">=", separated)  # restore this operator (comparison)
    separated = re.sub(
        "=  >", "=>", separated
    )  # restore this operator (when selector in VHDL)
    separated = re.sub("=  =", "==", separated)  # restore this operator (comparison)
    separated = re.sub("/  =", "/=", separated)  # restore this operator (comparison)
...
def surround_character_by_blanks(character, all_port_declarations_without_comments):
    ...
    return re.sub(
        search_character, " " + character + " ", all_port_declarations_without_comments
    )
...
def get_all_declared_constant_names(all_signal_declarations):
    all_signal_declarations_without_comments = remove_comments_and_returns(
        all_signal_declarations
    )
    all_signal_declarations_separated = surround_character_by_blanks(
