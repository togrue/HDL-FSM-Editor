...
def evaluate_commandline_parameters():
    ...
    argument_parser.add_argument(
        "-no_version_check",
        action="store_true",
        help="HDL-FSM-Editor will not check for a newer version at start.",
    )
    argument_parser.add_argument(
        "-no_message",
        action="store_true",
        help="HDL-FSM-Editor will not check for a message at start.",
    )
    argument_parser.add_argument(
        "-generate_hdl",
        action="store_true",
        help="HDL-FSM-Editor will generate HDL and exit.",
    )
    ...
    if args.filename is not None:
        if not exists(args.filename):
            messagebox.showerror(
                "Error in HDL-FSM-Editor", "File " + args.filename + " was not found."
            )
            ...
            messagebox.showerror(
                ...
                "File "
                + args.filename
                + " cannot be read. Must have extension '.hfe'.",
            )
        else:
...
def create_menu_bar():
    ...
    file_menu_button = ttk.Menubutton(
        menue_frame, text="File", style="Window.TMenubutton"
    )
    ...
    file_menu.add_command(
        label="New",
        accelerator="Ctrl+n",
        command=file_handling.remove_old_design,
        font=("Arial", 10),
    )
    file_menu.add_command(
        label="Open ...",
        accelerator="Ctrl+o",
        command=file_handling.open_file,
        font=("Arial", 10),
    )
    file_menu.add_command(
        label="Open Version 1 file ...",
        command=file_handling.open_file_old,
        font=("Arial", 10),
    )
    file_menu.add_command(
        label="Save",
        accelerator="Ctrl+s",
        command=file_handling.save,
        font=("Arial", 10),
    )
    file_menu.add_command(
        label="Save as ...", command=file_handling.save_as, font=("Arial", 10)
    )
    ...
    hdl_menu_button = ttk.Menubutton(
        menue_frame, text="HDL", style="Window.TMenubutton"
    )
    ...
    hdl_menu.add_command(
        label="Compile",
        accelerator="Ctrl+p",
        command=compile_handling.compile_hdl,
        font=("Arial", 10),
    )
    ...
    search_button = ttk.Button(
        search_frame,
        text="Find",
        command=lambda: canvas_editing.find(
            search_string, replace_string, replace=False
        ),
        style="Find.TButton",
    )
    search_string_entry = ttk.Entry(search_frame, width=23, textvariable=search_string)
    replace_string_entry = ttk.Entry(
        search_frame, width=23, textvariable=replace_string
    )
    replace_button = ttk.Button(
        search_frame,
        text="Find & Replace",
        command=lambda: canvas_editing.find(
            search_string, replace_string, replace=True
        ),
        style="Find.TButton",
    )
    search_string_entry.bind(
        "<Return>",
        lambda event: canvas_editing.find(search_string, replace_string, replace=False),
    )
    search_button.bind(
        "<Return>",
        lambda event: canvas_editing.find(search_string, replace_string, replace=False),
    ...
    replace_string_entry.bind(
        "<Return>",
        lambda event: canvas_editing.find(search_string, replace_string, replace=True),
    )
    replace_button.bind(
        "<Return>",
        lambda event: canvas_editing.find(search_string, replace_string, replace=True),
    ...
    info_menu_button = ttk.Menubutton(
        menue_frame, text="Info", style="Window.TMenubutton"
    )
    info_menu = tk.Menu(info_menu_button)
    info_menu_button.configure(menu=info_menu)
    info_menu.add_command(
        label="About",
        command=lambda: messagebox.showinfo("About:", header_string),
        font=("Arial", 10),
    )
    ...
    root.bind_all(
        "<Control-g>",
        lambda event: hdl_generation.run_hdl_generation(write_to_file=True),
    )
...
def create_control_notebook_tab():
    ...
    reset_signal_name_label = ttk.Label(
        control_frame, text="Name of asynchronous reset input port:", padding=5
    )
    reset_signal_name_entry = ttk.Entry(
        control_frame, width=23, textvariable=reset_signal_name
    )
    reset_signal_name_entry.bind(
    ...
    edit_cmd_label = ttk.Label(
        control_frame, text="Edit command (executed by Ctrl+e):", padding=5
    )
    ...
    working_directory_entry = ttk.Entry(
        control_frame, textvariable=working_directory_value, width=80
    )
    ...
    diagram_background_color_entry = ttk.Entry(
        control_frame, textvariable=diagram_background_color, width=80
    )
    diagram_background_color_button = ttk.Button(
        control_frame,
        text="Choose color...",
        command=run_color_changer,
        style="Path.TButton",
    )
...
def create_interface_notebook_tab():
    ...
    interface_package_text.bind(
        "<Control-Z>", lambda event: interface_package_text.edit_redo()
    )
    interface_package_text.bind(
    ...
    interface_generics_info = ttk.Label(
        interface_generics_frame, text="Undo/Redo: Ctrl-z/Ctrl-Z,Ctrl-y", padding=5
    )
    ...
    interface_generics_text.bind(
        "<Control-Z>", lambda event: interface_generics_text.edit_redo()
    )
    interface_generics_text.bind(
        "<<TextModified>>", lambda event: undo_handling.modify_window_title()
    )
    ...
    notebook.add(
        paned_window_interface, sticky=tk.N + tk.E + tk.W + tk.S, text="Interface"
    )
...
def create_internals_notebook_tab():
    ...
    internals_package_text.bind(
        "<Control-Z>", lambda event: internals_package_text.edit_redo()
    )
    internals_package_text.bind(
        "<<TextModified>>", lambda event: undo_handling.modify_window_title()
    )
    ...
    internals_architecture_text.bind(
        "<Control-z>", lambda event: internals_architecture_text.undo()
    )
    internals_architecture_text.bind(
        "<Control-Z>", lambda event: internals_architecture_text.redo()
    )
    internals_architecture_text.bind(
        "<<TextModified>>", lambda event: undo_handling.modify_window_title()
    )
    ...
    internals_architecture_frame.bind(
        "<Configure>", __resize_event_internals_tab_frames
    )
    ...
    interface_process_clocked_info = ttk.Label(
        internals_process_clocked_frame,
        text="Undo/Redo: Ctrl-z/Ctrl-Z,Ctrl-y",
        padding=5,
    )
    ...
    internals_process_clocked_text.bind(
        "<Control-z>", lambda event: internals_process_clocked_text.undo()
    )
    internals_process_clocked_text.bind(
        "<Control-Z>", lambda event: internals_process_clocked_text.redo()
    )
    internals_process_clocked_text.bind(
        "<<TextModified>>", lambda event: undo_handling.modify_window_title()
    )
    ...
    notebook.add(
        paned_window_internals, sticky=tk.N + tk.E + tk.W + tk.S, text="Internals"
    )
...
def create_diagram_notebook_tab():
    ...
    diagram_frame.grid()
    diagram_frame.columnconfigure(
        0, weight=1
    )  # tkinter method (grid_columnconfigure is tcl method)
    ...
    state_action_default_button = ttk.Button(
        ...
        style="DefaultStateActions.TButton",
    )
    global_action_clocked_button = ttk.Button(
        action_frame,
        text="Global Actions (clocked)",
        style="GlobalActionsClocked.TButton",
    )
    global_action_combinatorial_button = ttk.Button(
        action_frame,
        text="Global Actions (combinatorial)",
        style="GlobalActionsCombinatorial.TButton",
    )
    ...
    new_transition_button = ttk.Button(
        button_frame, text="new Transition", style="NewTransition.TButton"
    )
    new_state_button = ttk.Button(
        button_frame, text="new State", style="NewState.TButton"
    )
    new_connector_button = ttk.Button(
        button_frame, text="new Connector", style="NewConnector.TButton"
    )
    reset_entry_button = ttk.Button(
        button_frame, text="Reset Entry", style="ResetEntry.TButton"
    )
    ...
    root.bind_all(
        "<Escape>", lambda event: canvas_modify_bindings.switch_to_move_mode()
    )
    new_state_button.bind(
        "<Button-1>", lambda event: canvas_modify_bindings.switch_to_state_insertion()
    )
    new_transition_button.bind(
        "<Button-1>",
        lambda event: canvas_modify_bindings.switch_to_transition_insertion(),
    )
    new_connector_button.bind(
        "<Button-1>",
        lambda event: canvas_modify_bindings.switch_to_connector_insertion(),
    )
    reset_entry_button.bind(
        "<Button-1>",
        lambda event: canvas_modify_bindings.switch_to_reset_entry_insertion(),
    )
    state_action_default_button.bind(
        "<Button-1>",
        lambda event: canvas_modify_bindings.switch_to_state_action_default_insertion(),
    )
    global_action_clocked_button.bind(
        "<Button-1>",
        lambda event: canvas_modify_bindings.switch_to_global_action_clocked_insertion(),
    )
    global_action_combinatorial_button.bind(
        "<Button-1>",
        lambda event: canvas_modify_bindings.switch_to_global_action_combinatorial_insertion(),
    )
    view_area_button.bind(
        "<Button-1>", lambda event: canvas_modify_bindings.switch_to_view_area()
    )
    ...
    canvas.bind(
        "<Control-MouseWheel>", canvas_editing.zoom_wheel
    )  # MouseWheel used at Windows.
    canvas.bind(
        "<Control-Button-4>", canvas_editing.zoom_wheel
    )  # MouseWheel-Scroll-Up used at Linux.
    canvas.bind(
        "<Control-Button-5>", canvas_editing.zoom_wheel
    )  # MouseWheel-Scroll-Down used at Linux.
...
def create_hdl_notebook_tab():
    ...
    hdl_frame_text = custom_text.CustomText(
        hdl_frame, text_type="generated", undo=False, font=("Courier", 10)
    )
...
def cursor_move_hdl_tab(*_):
    ...
    index_string = hdl_frame_text.index(
        f"@{delta_x},{delta_y}"
    ...
    if line_number != line_number_under_pointer_hdl_tab:
        ...
        if (
            selected_file
            in link_dictionary.LinkDictionary.link_dict_reference.link_dict
            ...
            if (
                ...
                hdl_frame_text.tag_add(
                    "underline",
                    str(line_number) + "." + str(start_index - 1),
                    str(line_number + 1) + ".0",
                )
...
def cursor_move_log_tab(*_):
    ...
    if (
        ...
        content_of_line = log_frame_text.get(
            str(line_number) + ".0", str(line_number + 1) + ".0"
        )
        ...
        if match_object_of_message is not None:
            file_name = re.sub(
                regex_message_find, regex_file_name_quote, content_of_line
            )
            if debug:
                print("Regex found line                   : ", content_of_line)
                print("Regex found filename (group 1)     :", '"' + file_name + '"')
            try:
                file_line_number_string = re.sub(
                    regex_message_find, regex_file_line_number_quote, content_of_line
                )
                if file_line_number_string != content_of_line:
                    file_line_number = int(file_line_number_string)
                    if debug:
                        print(
                            "Regex found line-number (group 2)  :",
                            '"' + file_line_number_string + '"',
                        )
                else:
                    if debug:
                        print(
                            "Regex found no line-number         : Getting line-number by group 2 did not work."
                        )
                    return
                if (
                    file_name
                    ...
                    if (
                        ...
                        if debug:
                            print(
                                "Filename and line-number are found in Link-Dictionary."
                            )
                        log_frame_text.tag_add(
                            "underline",
                            str(line_number) + ".0",
                            str(line_number + 1) + ".0",
                        )
                        ...
                        if debug:
                            print(
                                "Filename is found in Link-Dictionary but line-number not."
                            )
    ...
    if notebook.index(notebook.select()) == 4:
        if language.get() == "VHDL":
            if select_file_number_text.get() == 1:
                hdlfilename = (
                    generate_path_value.get() + "/" + module_name.get() + ".vhd"
                )
                hdlfilename2 = ""
            else:
                hdlfilename = (
                    generate_path_value.get() + "/" + module_name.get() + "_e.vhd"
                )
                hdlfilename2 = (
                    generate_path_value.get() + "/" + module_name.get() + "_fsm.vhd"
                )
        else:  # verilog
        ...
        if language.get() != "VHDL" or select_file_number_text.get() == 1:
            if os.path.isfile(
                hdlfilename
            ) and date_of_hdl_file_shown_in_hdl_tab < os.path.getmtime(hdlfilename):
                ...
                if answer == "yes":
                    ...
                    date_of_hdl_file_shown_in_hdl_tab = (
            ...
            if (
                ...
                if answer == "yes":
                    ...
                    date_of_hdl_file2_shown_in_hdl_tab = (
