...
def move_to(event_x, event_y, transition_id, point, first, move_list, last) -> None:
    ...
    if point == "start":
        ...
        project_manager.canvas.coords(transition_tag, *transition_coords[0:2], event_x, event_y, *transition_coords[4:])
    elif point == "next_to_end":
    ...
    if ids != ():
        for canvas_id in ids:
            ...
            if (
                ...
                start_object_coords = project_manager.canvas.coords(tag_of_object_where_transition_starts)
                ...
                project_manager.root.unbind_all("<Escape>")
...
def draw_transition(coords, tags) -> int:
    transition_id = project_manager.canvas.create_line(coords, arrow="last", fill="blue", smooth=True, tags=tags)
...
def _add_tags_to_end_state_and_transition(end_state_canvas_id) -> None:
    project_manager.canvas.addtag_withtag("transition" + str(transition_number) + "_end", end_state_canvas_id)
    state_tags = project_manager.canvas.gettags(end_state_canvas_id)
    for tag in state_tags:
        if (tag.startswith("state") and not tag.endswith("_comment_line_end")) or tag.startswith("connector"):
            end_state_tag = tag
            break
    project_manager.canvas.addtag_withtag("going_to_" + end_state_tag, "transition" + str(transition_number))
...
def _abort_inserting_transition(
    ...
    project_manager.canvas.dtag(tag_of_object_where_transition_starts, tag_to_delete)
...
def _add_priority_rectangle_to_the_new_transition(transition_coords, start_state_canvas_id) -> None:
    ...
    priority_number_bbox_coords = project_manager.canvas.bbox(canvas_id_priority_text)
    canvas_id_priority_rectangle = draw_priority_rectangle(priority_number_bbox_coords, tag_of_priority_rectangle)

    project_manager.canvas.itemconfigure(canvas_id_priority_text, state=priority_visibility)
    project_manager.canvas.itemconfigure(canvas_id_priority_rectangle, state=priority_visibility)
    project_manager.canvas.tag_raise(tag_of_priority_number)
...
def draw_priority_rectangle(coords, tags):
    canvas_id_priority_rectangle = project_manager.canvas.create_rectangle(coords, tag=tags, fill=constants.STATE_COLOR)
...
def _configure_visibility_of_priority_rectangles(priority_dict):
    if len(priority_dict) == 1:
        ...
        for outgoing_transition in priority_dict:
            project_manager.canvas.itemconfigure(outgoing_transition + "priority", state=tk.NORMAL)
            project_manager.canvas.itemconfigure(outgoing_transition + "rectangle", state=tk.NORMAL)
