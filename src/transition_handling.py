...
def move_to(event_x, event_y, transition_id, point, first, move_list, last):
    ...
    if main_window.canvas.type(move_list[0][0]) == "line" and (
        move_list[0][1] in ("start", "end")
    ):
    ...
    if middle_of_line_is_moved is True:
        if first is True:
            ...
            if point == "start":
                ...
                return
            _difference_x, _difference_y = (
                -event_x + point_to_move[0],
                -event_y + point_to_move[1],
            )
    else:
        _difference_x = 0
        _difference_y = 0
    ...
    if last is True:
        event_x = canvas_editing.state_radius * round(
    ...
    if point == "start":
        main_window.canvas.coords(
            transition_tag, event_x, event_y, *transition_coords[2:]
        )
    elif point == "next_to_start":
        main_window.canvas.coords(
            transition_tag,
            *transition_coords[0:2],
            event_x,
            event_y,
            *transition_coords[4:],
        )
        ...
        main_window.canvas.coords(
            transition_tag, *transition_coords[-8:-2], event_x, event_y
        )
    else:
        print("transition_handling: Fatal, unknown point =", point)
    ...
    if transition_tag.startswith(
        "transition"
    ):  # There is no priority rectangle at a "connection".
        ...
        if point == "start":
            if (
                start_state_coords == []
                or main_window.canvas.type(transition_tag + "_start") == "polygon"
                ...
                start_state_radius = (
                    abs(start_state_coords[2] - start_state_coords[0]) / 2
                )
            ...
            [priority_middle_x, priority_middle_y, _, _] = (
                vector_handling.shorten_vector(
        ...
        [rectangle_width_half, rectangle_height_half] = _get_rectangle_dimensions(
        ...
        main_window.canvas.coords(
            transition_tag + "priority", priority_middle_x, priority_middle_y
        )
        main_window.canvas.tag_raise(transition_tag + "rectangle", transition_tag)
        main_window.canvas.tag_raise(
            transition_tag + "priority", transition_tag + "rectangle"
        )


def _get_rectangle_dimensions(canvas_id):
...
def get_point_to_move(item_id, event_x, event_y):
    ...
    for i in range(number_of_points):
        distance_event_to_point.append(
            math.sqrt(
                (event_x - transition_coords[2 * i]) ** 2
                + (event_y - transition_coords[2 * i + 1]) ** 2
            )
        )
    ...
    if number_of_points == 4:
        ...
        if distance_event_to_point[2] < distance_event_to_point[0]:
            ...
            main_window.canvas.coords(
                item_id,
                *transition_coords[0:4],
                event_x,
                event_y,
                *transition_coords[4:6],
...
def _remove_duplicate_points(transition_coords):
        ...
        for canvas_id in ids:
            ...
            if (
                ...
                or (
                    ...
                    and _reset_entry_has_no_transition(canvas_id)
                )
                or (
                    element_type == "rectangle"
                    and main_window.canvas.gettags(canvas_id)[0].startswith("connector")
                )
            ):
                for tag in main_window.canvas.gettags(canvas_id):
                    if (
                        ...
                        main_window.canvas.addtag_withtag(
                            "transition" + str(transition_number) + "_start",
                            tag_of_object_where_transition_starts,
                        )
                start_object_coords = main_window.canvas.coords(
                    tag_of_object_where_transition_starts
                )
                if element_type in ["oval", "rectangle"]:
                    line_start_x = (
                        start_object_coords[0] / 2 + start_object_coords[2] / 2
                    )
                    line_start_y = (
                        start_object_coords[1] / 2 + start_object_coords[3] / 2
                    )
                ...
                main_window.canvas.tag_bind(
                    ...
                    "<Enter>",
                    lambda event,
                    transition_id=transition_id: main_window.canvas.itemconfig(
                        transition_id, width=3
                    ),
                )
                main_window.canvas.tag_bind(
                    transition_id,
                    ...
                    lambda event,
                    transition_id=transition_id: main_window.canvas.itemconfig(
                        transition_id, width=1
                    ),
                )
                main_window.canvas.tag_bind(
                    transition_id,
                    "<Button-3>",
                    lambda event, transition_id=transition_id: show_menu(
                        event, transition_id
                    ),
                )
                ...
                transition_draw_funcid = main_window.canvas.bind(
                    "<Motion>",
                    lambda event, transition_id=transition_id: _transition_draw(
                        event, transition_id
                    ),
                    add="+",
                )
                ...
                main_window.root.bind_all(
                    ...
                    tag_to_delete="transition"
                    + str(transition_number)
                    + "_start": _abort_inserting_transition(
                        transition_id,
...
def _reset_entry_has_no_transition(canvas_id):
...
def _transition_draw(event, canvas_id):
    [event_x, event_y] = (
        canvas_editing.translate_window_event_coordinates_in_exact_canvas_coordinates(
            event
        )
...
def _handle_next_added_transition_point(
    event, transition_id, start_state_canvas_id, transition_draw_funcid
):
    ...
    end_state_canvas_id = (
        _get_canvas_id_of_state_or_connector_under_new_transition_point(event_x, event_y)
    )
    transition_ends_at_connector = _check_if_transition_ends_at_connector(
        end_state_canvas_id
    )
    if end_state_canvas_id is None:
        if (
            len(transition_coords) < 8
        ):  # An additional intermediate point is added to the transition.
        ...
        "coming_from_reset_entry" in transition_tags
        and transition_ends_at_connector is True
    ):
        return
        ...
        transition_coords = (
            _move_transition_start_and_end_point_to_the_edge_of_the_state_circle(
        ...
        _add_priority_rectangle_to_the_new_transition(
            transition_coords, start_state_canvas_id
        )
        transition_number += 1
        undo_handling.design_has_changed()


def _check_if_transition_ends_at_connector(end_state_canvas_id):
...
def _get_canvas_id_of_state_or_connector_under_new_transition_point(event_x, event_y):
    for canvas_id in main_window.canvas.find_overlapping(
...
def _add_tags_to_end_state_and_transition(end_state_canvas_id):
    main_window.canvas.addtag_withtag(
        "transition" + str(transition_number) + "_end", end_state_canvas_id
    )
    ...
    for tag in state_tags:
        if (
            ...
            break
    main_window.canvas.addtag_withtag(
        "going_to_" + end_state_tag, "transition" + str(transition_number)
    )
...
def _end_transition_insertion_by_modifying_bindings(transition_draw_funcid):
    # Restore bindings:
    main_window.root.unbind_all("<Escape>")
...
def _add_priority_rectangle_to_the_new_transition(
    transition_coords, start_state_canvas_id
):
    ...
    if len(priority_dict) == 1:
        ...
        for outgoing_transition in priority_dict:
            main_window.canvas.itemconfigure(
                outgoing_transition + "priority", state=tk.NORMAL
            )
            main_window.canvas.itemconfigure(
                outgoing_transition + "rectangle", state=tk.NORMAL
            )
    ...
    main_window.canvas.itemconfigure(
        tag_of_new_transition + "priority", state=transition_priority_visibility
    )
    ...
    main_window.canvas.tag_bind(
        ...
        "<Double-Button-1>",
        lambda event, transition_tag=tag_of_new_transition: edit_priority(
...
def determine_priorities_of_outgoing_transitions(start_state_canvas_id):
    ...
    for tag in all_tags:
        if tag.startswith("transition") and tag.endswith("_start"):
            ...
            priority_dict[transition_tag] = main_window.canvas.itemcget(
                transition_tag + "priority", "text"
            )
    return priority_dict


def _get_unused_priority(priority_dict):
...
def edit_priority(event, transition_tag):
    ...
    text_box.bind(
        "<Return>",
        lambda event, transition_tag=transition_tag, text_box=text_box: _update_priority(
    ...
    text_box.bind(
        ...
        text_box=text_box,
        old_text=old_text: _abort_edit_text(transition_tag, text_box, old_text),
    )
    [event_x, event_y] = (
        canvas_editing.translate_window_event_coordinates_in_exact_canvas_coordinates(
            event
    ...
    main_window.canvas.create_window(
        event_x, event_y, window=text_box, tag="entry-window"
    )
    text_box.focus_set()


def _update_priority(transition_tag, text_box):
...
def _abort_edit_text(transition_tag, text_box, old_text):
    main_window.canvas.delete("entry-window")
...
def show_menu(event, transition_id):
    ...
    listbox.bind(
        ...
        menu_x=event_x,
        menu_y=event_y,
        transition_id=transition_id: _evaluate_menu(
            event, window, listbox, menu_x, menu_y, transition_id
        ),
    )
    listbox.bind(
        "<Leave>",
        lambda event, window=window, listbox=listbox: _close_menu(
            event, window, listbox
        ),
    )


def _evaluate_menu(event, window, listbox, menu_x, menu_y, transition_id):
...
def _close_menu(event, window, listbox):
    listbox.destroy()
    main_window.canvas.delete(window)
