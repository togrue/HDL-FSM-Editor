...
class ListSeparationCheck:
    def __init__(self, list_string: str, language: str) -> None:
        ...
        if language == "VHDL":
            ...
            separator = ","
            comment_identifier = "//"
        list_string_without_block_comment = hdl_generation_library.remove_vhdl_block_comments(list_string)
        list_string_without_comments = self._replace_all_comments_at_line_end(
            list_string_without_block_comment, comment_identifier
        )
        self._remove_illegal_separator(list_string_without_comments, separator)

    def get_fixed_list(self) -> str:
        return self.list_string

    def _replace_all_comments_at_line_end(self, list_string_without_block_comment: str, comment_identifier: str) -> str:
        list_array = list_string_without_block_comment.split("\n")
        list_string_without_comments = ""
        for line in list_array:
            list_string_without_comments += self._replace_comment_at_line_end_by_blank(comment_identifier, line) + "\n"
        return list_string_without_comments[:-1]  # remove last return

    def _replace_comment_at_line_end_by_blank(self, comment_identifier: str, line: str) -> str:
        match_object = re.search(comment_identifier + ".*", line)
        if match_object is not None:
            line = (
                line[: match_object.span()[0]]
                + " " * (match_object.span()[1] - match_object.span()[0])
                + line[match_object.span()[1] :]
            )
        return line

    def _remove_illegal_separator(self, list_string_without_comments: str, separator: str) -> None:
        for index, char in enumerate(reversed(list_string_without_comments)):
            if char not in (" ", "\n"):
                if char == separator:
                    self._remove_character_by_blank(index)
                break

    def _remove_character_by_blank(self, index: int) -> None:
        if index == 0:
