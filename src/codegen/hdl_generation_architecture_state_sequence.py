        ...
        if transition_specification["command"] == "when":
            ...
            if "state_comments" in transition_specification:
                state_comments = re.sub(
                    r"^",
                    "    -- ",
                    transition_specification["state_comments"],
                    flags=re.M,
                )
                state_comments = state_comments[
                    :-7
                ]  # remove blanks and the comment-string from the last empty line.
            ...
            for entry in transition_specification["actions"]:
                ...
                for index, single_action in enumerate(action_list):
                    ...
                    if single_action_raw.startswith(
                        "if"
                    ) or single_action_raw.startswith("end if"):
            ...
            if transition_specification["target"] != "":
                vhdl.append(
                    " " * 4 + "state <= " + transition_specification["target"] + ";\n"
                )
                ignore_control_for_vhdl_indent.append(False)
                file_line_number += 1
...
def create_verilog_for_the_state_sequence(
        ...
        if transition_specification["command"] == "when":
            ...
            if transition_condition.count("\n") == 0:
                if "//" in transition_condition:
                    ...
                    verilog.append(
                        "if (" + transition_specification["condition"] + ") begin\n"
                    )
                ignore_control_for_verilog_indent.append(False)
                link_dictionary.LinkDictionary.link_dict_reference.add(
                    file_name,
            ...
            if transition_condition.count("\n") == 0:
                verilog.append(
                    "end else if ("
                    + transition_specification["condition"]
                    + ") begin\n"
                )
                ignore_control_for_verilog_indent.append(False)
                link_dictionary.LinkDictionary.link_dict_reference.add(
            ...
            for entry in transition_specification["actions"]:
                ...
                for index, single_action in enumerate(action_list):
                    ...
                    if single_action_raw.startswith(
                        "if"
                    ) or single_action_raw.startswith("end"):
                        # Because this control-statement was inserted by the user it must be handled as action at the indent process later.
    ...
    for index, verilog_line in enumerate(verilog):
        if (
            verilog_line.startswith("if") or verilog_line.endswith(": begin\n")
        ) and not ignore_control_for_verilog_indent[index]:
            indent += 1
        verilog_indented += " " * 4 * indent + verilog_line
        if (
            verilog_line.startswith("end")
            and not verilog_line.startswith("end else if ")
            and not verilog_line.startswith("end else begin")
        ) and not ignore_control_for_verilog_indent[index]:
            indent -= 1
