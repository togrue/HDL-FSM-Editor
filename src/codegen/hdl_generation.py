...
def _design_is_not_ready_for_hdl_generation():
    ...
    if reset.isspace() or reset == "":
        messagebox.showerror(
            "Error in HDL-FSM-Editor",
            "No reset signal name is specified,\nso no HDL-generation is possible.",
        )
        return True
    ...
    if clock.isspace() or clock == "":
        messagebox.showerror(
            "Error in HDL-FSM-Editor",
            "No clock signal name is specified,\nso no HDL-generation is possible.",
        )
        return True
    return False
...
def _create_entity(file_name, file_line_number):
    ...
    port_declarations = list_separation_check.ListSeparationCheck(
        port_declarations, "VHDL"
    ).get_fixed_list()
...
def _create_module_ports(file_name, file_line_number):
    ...
    parameters = list_separation_check.ListSeparationCheck(
        parameters, "Verilog"
    ).get_fixed_list()
    if parameters != "":
        parameters = (
    ...
    ports = hdl_generation_library.get_text_from_text_widget(
        main_window.interface_ports_text
    )
...
def _write_hdl_file(
    ...
    if main_window.select_file_number_text.get() == 1:
        ...
        if write_to_file:
            ...
            fileobject.close()
        last_line_number_of_file1 = (
            content.count("\n") + 1
        )  # For example: 3 lines are separated by 2 returns.
        main_window.size_of_file1_line_number = (
        ...
        if write_to_file:
            fileobject_architecture = open(
                path_name_architecture, "w", encoding="utf-8"
            )
        ...
        for tag in main_window.canvas.gettags(canvas_id):
            if reg_ex_for_state_tag.match(tag):
                ...
                if not single_element_list:
                    ...
                    reference_to_state_comment_window = (
                        state_comment.StateComment.dictionary[single_element_list[0]]
                    )
                    state_comments = reference_to_state_comment_window.text_id.get(
                        "1.0", "end - 1 chars"
                    )
                    ...
                    if first_line_of_state_comments == "":
                        ...
                        if not first_line_is_a_number:
                            ...
                            if (
                                int(first_line_of_state_comments)
                                in state_tag_dict_with_prio
                            ):
                                state_tag_list.append(tag)
