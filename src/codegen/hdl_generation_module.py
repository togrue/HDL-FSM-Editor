...
def create_module_logic(file_name, file_line_number, state_tag_list_sorted):
    ...
    architecture += hdl_generation_library.indent_text_by_the_given_number_of_tabs(
        1, state_signal_type_definition
    )
    ...
    )  # No return after the last line of the condition
    link_dictionary.LinkDictionary.link_dict_reference.add(
        file_name,
        file_line_number,
    ...
    file_line_number += (
        1  # There is always a "state <=  ..." assignment which must be skipped
    )
    ...
    global_actions_before_reference, global_actions_before = (
    ...
    transition_specifications = (
        hdl_generation_library.extract_transition_specifications_from_the_graph(
            state_tag_list_sorted
        )
    )
    state_sequence, file_line_number = (
        hdl_generation_architecture_state_sequence.create_verilog_for_the_state_sequence(
    ...
    global_actions_after_reference, global_actions_after = (
    ...
    state_actions_process, file_line_number = (
        hdl_generation_architecture_state_actions.create_state_action_process(
            file_name, file_line_number, state_tag_list_sorted
        )
    )
    architecture += hdl_generation_library.indent_text_by_the_given_number_of_tabs(
        1, state_actions_process
    )

    concurrent_actions_reference, concurrent_actions = (
        hdl_generation_library.create_concurrent_actions()
    )
    if concurrent_actions != "":
        concurrent_actions = "// Global Actions combinatorial:\n" + concurrent_actions
        file_line_number += 1
        architecture += hdl_generation_library.indent_text_by_the_given_number_of_tabs(
            1, concurrent_actions
        )
...
def _create_signal_declaration_for_the_state_variable(state_tag_list_sorted):
    ...
    if main_window.language.get() == "Verilog":
        ...
        for name in list_of_all_state_names:
            name_list += name + ", "
        signal_declaration = (
            "typedef enum {" + name_list[:-2] + "} t_state;\nt_state state;\n"
        )
...
def _get_reset_edge(reset_condition):
    ...
    if reset_condition_mod.endswith("1'b0"):
        ...
        messagebox.showerror(
            "Error",
            "The reset polarity could not be determined from the reset condition: "
            + reset_condition_mod,
        )
        return "unknown_edge"
