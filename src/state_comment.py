...
class StateComment:
    ...
    def __init__(
        ...
        StateComment.dictionary[self.window_id] = (
            self  # Store the object-reference with the Canvas-id as key.
        )
    ...
    def add_line(
        self, menu_x, menu_y, state_identifier
    ):  # Called by state_handling.evaluate_menu().
    ...
    def tag(self, state_identifier):  # Called by state_handling.evaluate_menu().
        main_window.canvas.addtag_withtag(
            state_identifier + "_comment_line_end", state_identifier
        )
        main_window.canvas.itemconfigure(
            self.window_id,
            tag=(
                state_identifier + "_comment",
                state_identifier + "_comment_line_start",
            ),
        )
