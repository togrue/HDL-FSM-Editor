...
class StateComment:
    ...
    def __init__(
        ...
        self.window_id: int = main_window.canvas.create_window(
            float(menu_x + 100), float(menu_y), window=self.frame_id, anchor=tk.W
        )
        ...
        self.label_id.grid(column=0, row=0, sticky=f"{tk.N}{tk.W}{tk.E}")
    ...
    def __draw_polygon_around_window(self) -> None:
        ...
        if self.move_rectangle is not None:
            main_window.canvas.tag_bind(
                ...
                "<Leave>",
                lambda event: (
                    main_window.canvas.delete(self.move_rectangle) if self.move_rectangle is not None else None
                ),
            )
    ...
    def add_line(self, menu_x, menu_y, state_identifier) -> None:  # Called by state_handling.evaluate_menu().
        ...
        self.line_id = main_window.canvas.create_line(
            float(menu_x + 100),
            float(menu_y),
            float((state_coords[2] + state_coords[0]) / 2),
            float((state_coords[3] + state_coords[1]) / 2),
            dash=(2, 2),
