...
def create_architecture(file_name, file_line_number, state_tag_list_sorted):
    architecture = ""
    ...
    architecture += (
        "        elsif rising_edge(" + main_window.clock_signal_name.get() + ") then\n"
    )
    link_dictionary.LinkDictionary.link_dict_reference.add(
        file_name,
        file_line_number,
    ...
    state_sequence, file_line_number = (
        hdl_generation_architecture_state_sequence.create_vhdl_for_the_state_sequence(
    ...
    reference_to_concurrent_actions_custom_text, concurrent_actions = (
