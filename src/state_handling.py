...
def insert_state(event):
    ...
    main_window.canvas.tag_bind(
        text_id,
        "<Double-Button-1>",
        lambda event, text_id=text_id: edit_state_name(event, text_id),
    )
    main_window.canvas.tag_bind(
        state_id,
        "<Enter>",
    ...
    main_window.canvas.tag_bind(
        state_id, "<Button-3>", lambda event, id=state_id: show_menu(event, id)
    )
    ...
    if selected_entry == "add action":
        ...
        if not action_block_exists:
            action_ref = state_action_handling.MyText(
                menu_x, menu_y, height=1, width=8, padding=3, increment=True
            )
        ...
        for tag in tags:
            if tag.startswith("state"):
                ...
                comment_ref = state_comment.StateComment(
                    menu_x, menu_y, height=1, width=8, padding=3
                )
