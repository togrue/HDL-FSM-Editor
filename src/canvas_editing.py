        ...
        if main_window.canvas.type(i) is None:
            ...
            for tag in tags_of_item_i:
                ...
                if tag.startswith('state_actions_default'):
                    ...
                    main_window.canvas.dtag(tag.replace("_comment", ""), tag + "_line_end") # delete at state: "state"<integer>"_comment_line_end"
...
def view_area_by_button3(rectangle_id):
    ...
    if rectangle_coords[0]!=rectangle_coords[2] and rectangle_coords[1]!=rectangle_coords[3]:
        ...
        overlapping_canvas_ids = main_window.canvas.find_overlapping(rectangle_coords[0]-5, rectangle_coords[1]-5, rectangle_coords[0]+5, rectangle_coords[1]+5)
        item_found = False
...
def shift_visible_center_to_window_center(new_visible_center_string):
    ...
    actual_visible_rectangle = [main_window.canvas.canvasx(0), main_window.canvas.canvasy(0),
                                main_window.canvas.canvasx(main_window.canvas.winfo_width()), main_window.canvas.canvasy(main_window.canvas.winfo_height())]
...
def find(search_string, replace_string, replace):
    ...
    for item in all_canvas_items:
        if main_window.canvas.type(item)=="window":
            ...
            number_of_hits = search_in_text_fields_of_canvas_window(search_pattern, item, text_ids, replace, replace_pattern)
            if number_of_hits==-1:
    ...
    if continue_search:
        if main_window.language.get()=="VHDL":
            internals_text_fields = [main_window.internals_package_text, main_window.internals_architecture_text,
...
def search_in_text_fields_of_canvas_window(search_pattern, canvas_window, text_ids_of_actions, replace, replace_pattern):
    number_of_hits_all = 0
    count = tk.IntVar()
...
def __show_menu(zoom_coords):
    ...
    menu = OptionMenu.MyListbox(main_window.canvas, canvas_menue_entries_list, height=2, bg='lightgrey', width=25, activestyle='dotbox', relief=tk.RAISED)
