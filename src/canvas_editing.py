    ...
    for i in canvas_ids:
        if project_manager.canvas.type(i) == "window":
            if i in state_action_handling.StateAction.mytext_dict:
                state_action_handling.StateAction.mytext_dict[i].label_id.configure(
                    font=("Arial", int(used_label_fontsize))
                )
                state_action_handling.StateAction.mytext_dict[i].text_id.configure(
                    font=("Courier", int(project_manager.fontsize))
                )
                for highlight_tag_name in constants.VHDL_HIGHLIGHT_PATTERN_DICT:
                    state_action_handling.StateAction.mytext_dict[i].text_id.tag_configure(
                        highlight_tag_name,
