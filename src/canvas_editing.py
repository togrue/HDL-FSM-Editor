...
def store_mouse_position(event):  # used by delete().
    ...
    [canvas_x_coordinate, canvas_y_coordinate] = (
        translate_window_event_coordinates_in_exact_canvas_coordinates(event)
    )
...
def translate_window_event_coordinates_in_rounded_canvas_coordinates(event):
    canvas_grid_x_coordinate = main_window.canvas.canvasx(
        event.x, gridspacing=state_radius
    )
    canvas_grid_y_coordinate = main_window.canvas.canvasy(
        event.y, gridspacing=state_radius
    )
...
def delete():
    ...
    ids = main_window.canvas.find_overlapping(
        canvas_x_coordinate - 2,
        canvas_y_coordinate - 2,
        canvas_x_coordinate + 2,
        canvas_y_coordinate + 2,
    )
    ...
    for i in ids:
        ...
        if main_window.canvas.type(i) is None:
            ...
            main_window.canvas.type(i) == "polygon"
        ):  # Only the reset_entry is a polygon.
            for tag in tags_of_item_i:  # Remove reset entry and connected transition.
                ...
                elif tag.startswith("transition") and tag.endswith(
                    "_start"
                ):  # transition<n>_start
                    transition = tag[0:-6]
            ...
            for tag in tags_of_item_i:
                ...
                if tag.startswith("state_actions_default"):
                    ref = state_actions_default.StateActionsDefault.dictionary[
                        item_id[0]
                    ]
                    del custom_text.CustomText.read_variables_of_all_windows[
                        ref.text_id
                    ]
                    del custom_text.CustomText.written_variables_of_all_windows[
                        ref.text_id
                    ]
                    ...
                    ref = condition_action_handling.ConditionAction.dictionary[
                        item_id[0]
                    ]
                    del custom_text.CustomText.read_variables_of_all_windows[
                        ref.condition_id
                    ...
                    ref = global_actions_combinatorial.GlobalActionsCombinatorial.dictionary[
                        item_id[0]
                    ]
                    del custom_text.CustomText.read_variables_of_all_windows[
                        ref.text_id
                    ...
                    main_window.global_action_combinatorial_button.config(
                        state=tk.NORMAL
                    )
            ...
            for tag in tags_of_item_i:
                if tag.startswith("state") and not tag.endswith("_comment_line_end"):
                ...
                elif tag.startswith("connection") and tag.endswith(
                    "_end"
                ):  # connection<n>_end
                    connection = tag[0:-4]
                    ...
                    main_window.canvas.delete(
                        connection + "_start"
                    )  # delete action window
                    main_window.canvas.dtag(
                        i, connection + "_end"
                    )  # delete connection<n>_end tag from state
                elif tag.startswith("transition") and tag.endswith(
                    "_end"
                ):  # transition<n>_end
                    transition = tag[0:-4]
                    ...
                    for transition_tag in transition_tags:
                        if transition_tag.startswith("coming_from_"):
                            ...
                            condition_action_tag = transition_tag[:-4]
                            main_window.canvas.delete(
                                condition_action_tag + "_anchor"
                            )  # delete the window
                            main_window.canvas.delete(
                                condition_action_tag
                            )  # delete the line to the window
                    main_window.canvas.delete(transition)
                    main_window.canvas.delete(
                        transition + "rectangle"
                    )  # delete priority rectangle
            ...
            i
        ) == "line" and "grid_line" not in main_window.canvas.gettags(
            i
            ...
            i
        ) == "line" and "grid_line" in main_window.canvas.gettags(
            i
...
def adapt_visibility_of_priority_rectangles_at_state(start_state):
    ...
    for start_state_tag in tags_of_start_state:
        if start_state_tag.startswith("transition") and start_state_tag.endswith(
            "_start"
        ):
    ...
    if number_of_outgoing_transitions == 1:
        main_window.canvas.itemconfigure(
            tag_of_outgoing_transition + "rectangle", state=tk.HIDDEN
        )
        main_window.canvas.itemconfigure(
            tag_of_outgoing_transition + "priority", state=tk.HIDDEN
        )


def start_view_rectangle(event):
    [event_x, event_y] = translate_window_event_coordinates_in_exact_canvas_coordinates(
        event
    )
    rectangle_id = main_window.canvas.create_rectangle(
        event_x, event_y, event_x, event_y, dash=(3, 5)
    )
    ...
    main_window.canvas.bind(
        "<ButtonRelease-3>", lambda event: _view_area_by_button3(rectangle_id)
    )


def _draw_view_rectangle(event, rectangle_id):  # Called by Motion-event.
    [event_x, event_y] = translate_window_event_coordinates_in_exact_canvas_coordinates(
        event
    )
    ...
    if event_x > rectangle_coords[0] and event_y > rectangle_coords[1]:
        main_window.canvas.coords(
            rectangle_id, rectangle_coords[0], rectangle_coords[1], event_x, event_y
        )


def _view_area_by_button3(rectangle_id):
    rectangle_coords = main_window.canvas.coords(rectangle_id)
    if (
        rectangle_coords[0] != rectangle_coords[2]
        and rectangle_coords[1] != rectangle_coords[3]
    ):
        _view_area(rectangle_id)
    else:
        main_window.canvas.delete(rectangle_id)
        overlapping_canvas_ids = main_window.canvas.find_overlapping(
            rectangle_coords[0] - 5,
            rectangle_coords[1] - 5,
            rectangle_coords[0] + 5,
            rectangle_coords[1] + 5,
        )
        item_found = False
...
def view_rectangle(complete_rectangle, check_fit):
    if (
        complete_rectangle[2] - complete_rectangle[0] != 0
        and complete_rectangle[3] - complete_rectangle[1] != 0
    ):
        visible_rectangle = [
...
def _decrement_font_size_if_window_is_too_wide():
    visible_rectangle = [
        main_window.canvas.canvasx(0),
        main_window.canvas.canvasy(0),
        main_window.canvas.canvasx(main_window.canvas.winfo_width()),
    ...
    if (
        ...
        and fontsize
        != 1  # When fontsize==1 then zoom_factor calculates to 0, which makes no sense.
    ):
        complete_center = _determine_center_of_rectangle(complete_rectangle)
        visible_center = _determine_center_of_rectangle(visible_rectangle)
        _move_canvas_point_from_to(complete_center, visible_center)
...
def _determine_center_of_rectangle(rectangle_coords):
    return [
        (rectangle_coords[0] + rectangle_coords[2]) / 2,
        (rectangle_coords[1] + rectangle_coords[3]) / 2,
    ]


def _move_canvas_point_from_to(complete_center, visible_center):
    main_window.canvas.scan_mark(int(complete_center[0]), int(complete_center[1]))
    main_window.canvas.scan_dragto(
        int(visible_center[0]), int(visible_center[1]), gain=1
    )
...
def _canvas_zoom(zoom_center, zoom_factor):
...
def _scroll_canvas_to_show_the_zoom_center(zoom_center, zoom_factor):
...
def _adapt_global_size_variables(factor):
    global state_radius
...
def _modify_font_sizes_of_all_canvas_items(factor):
    global fontsize
    ...
    for i in canvas_ids:
        if main_window.canvas.type(i) == "window":
            if i in state_action_handling.MyText.mytext_dict:
                state_action_handling.MyText.mytext_dict[i].label_id.configure(
                ...
                state_comment.StateComment.dictionary[i].label_id.configure(
                    font=("Arial", int(used_label_fontsize))
                )
                ...
                condition_action_handling.ConditionAction.dictionary[
                    i
                ].condition_label.configure(font=("Arial", int(used_label_fontsize)))
                condition_action_handling.ConditionAction.dictionary[
                    i
                ].action_label.configure(font=("Arial", int(used_label_fontsize)))
                condition_action_handling.ConditionAction.dictionary[
                    i
                ].condition_id.configure(font=("Courier", int(fontsize)))
                condition_action_handling.ConditionAction.dictionary[
                    i
                ].action_id.configure(font=("Courier", int(fontsize)))
                for keyword in main_window.keywords:
                    condition_action_handling.ConditionAction.dictionary[
                        i
                    ].condition_id.tag_configure(
                        keyword,
                        foreground=main_window.keyword_color[keyword],
                        font=("Courier", int(fontsize), "normal"),
                    )
                ...
                global_actions_combinatorial.GlobalActionsCombinatorial.dictionary[
                    i
                ].label.configure(font=("Arial", int(used_label_fontsize)))
                global_actions_combinatorial.GlobalActionsCombinatorial.dictionary[
                    i
                ].text_id.configure(font=("Courier", int(fontsize)))
                for keyword in main_window.keywords:
                    global_actions_combinatorial.GlobalActionsCombinatorial.dictionary[
                        i
                    ].text_id.tag_configure(
                        keyword,
                        foreground=main_window.keyword_color[keyword],
                        font=("Courier", int(fontsize), "normal"),
                    )
...
def find(search_string, replace_string, replace):
    ...
    if search_pattern == "":
        messagebox.showinfo(
            "HDL-FSM-Editor", "Search is aborted as you search for an empty string."
        )
        return
    ...
    for item in all_canvas_items:
        if main_window.canvas.type(item) == "window":
            text_ids = _get_text_ids_of_canvas_window(item)
            number_of_hits = _search_in_text_fields_of_canvas_window(
            ...
            number_of_hits = _search_in_canvas_text(
    ...
    if continue_search:
        if replace:
            undo_handling.design_has_changed()
            messagebox.showinfo(
                "HDL-FSM-Editor", "Number of replacements = " + str(number_of_hits_all)
            )
        else:
            messagebox.showinfo(
                "HDL-FSM-Editor", "Number of hits = " + str(number_of_hits_all)
            )


def _get_text_ids_of_canvas_window(item):
    text_ids = []
    if item in state_action_handling.MyText.mytext_dict:
        ...
        text_ids.append(
            condition_action_handling.ConditionAction.dictionary[item].condition_id
        )
        text_ids.append(
            condition_action_handling.ConditionAction.dictionary[item].action_id
        )
        ...
        text_ids.append(
            state_actions_default.StateActionsDefault.dictionary[item].text_id
        )
...
def _search_in_text_fields_of_canvas_window(
    ...
    for text_id in text_ids_of_actions:
        number_of_hits = _search_in_text_widget(
            text_id, search_pattern, count, canvas_window, replace, replace_pattern
        )
        if number_of_hits == -1:
...
def _search_in_text_widget(
    ...
    while True:
        ...
        if replace:
            ...
            text_id.tag_remove(
                "hit", index, index + " + " + str(count.get()) + " chars"
            )
            if continue_search is False:
                number_of_hits = -1
                break
        ...
        if start == index:
            messagebox.showinfo(
                "HDL-FSM-Editor",
                "Search in text widgets is aborted as for unknown reason no progress happens.",
            )
            break
    return number_of_hits


def _search_in_canvas_text(item, search_pattern, replace, replace_pattern):
    ...
    while True:
        ...
        if replace:
            ...
            start = len(
                text
            )  # The search-pattern cannot be found again in the next loop.
        else:
            number_of_hits += 1
            _move_in_foreground("Diagram")
        ...
        if start == hit_begin:
            messagebox.showinfo(
                "HDL-FSM-Editor",
                "Search in canvas text is aborted as for unknown reason no progress happens.",
            )
            break
    return number_of_hits
...
def _search_in_text_fields_of_a_tab(
    tab, kind, search_pattern, interface_text_fields, replace, replace_pattern
):
    count = tk.IntVar()
    number_of_hits = 0
    for text_id in interface_text_fields:
        ...
        while True:
            ...
            if start == index:
                messagebox.showinfo(
                    "HDL-FSM-Editor",
                    "Search in tab text fields is aborted as for unknown reason no progress happens.",
                )
                break
    return number_of_hits


def _move_in_foreground(tab):
...
def __show_menu(zoom_coords):
    ...
    menue_window = main_window.canvas.create_window(
        zoom_coords[0], zoom_coords[1], window=menu
    )
