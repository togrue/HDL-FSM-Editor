...
def _execute(command):
    ...
    try:
        ...
        messagebox.showerror(
            ...
            "FileNotFoundError caused by compile command:\n" + command_string,
        )
        return False
    return True
    ...
    for entry in command_array:
        if entry == "$file":
            ...
            if not exists(file_name):
                messagebox.showerror(
                    "Error",
                    "Compile is not possible, HDL file "
                    + file_name
                    + " does not exist.",
                )
                return
            ...
            if not exists(file_name1):
                messagebox.showerror(
                    "Error",
                    "Compile is not possible, as HDL file"
                    + file_name1
                    + " does not exist.",
                )
                return
...
def _insert_line_in_log(text):
    ...
    main_window.log_frame_text.config(state=tk.NORMAL)
    if (
        match_object_of_message is not None
        or "error" in text_low
        or "warning" in text_low
    ):
